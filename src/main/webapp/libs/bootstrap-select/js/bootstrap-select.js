...
!function($) {
    ...
    Selectpicker.prototype = {
        ...
        init: function(e) {
            ...
            this.$clearLink = this.$newElement.find('input.bootstrap-select-clearLink');
            this.$selectAllLink = this.$newElement.find('input.bootstrap-select-selectAllLink');
            ...
            this.clearLinkListener();
            this.selectAllLinkListener();
            this.render();
            this.liHeight();
            this.setStyle();
            this.setWidth();
        ...
        createDropdown: function() {
            ...
            var searchbox = this.options.liveSearch ? '<input type="text" class="form-control searchbox-input"/>' : '';
            var clearLink = this.options.clearLink ? '<input type="submit" class="input-block-level btn btn-link bootstrap-select-clearLink"/>' : '';
            var selectAllLink = this.options.selectAllLink ? '<input type="submit" class="input-block-level btn btn-link bootstrap-select-selectAllLink"/>' : '';

            var boxes = '<div class="bootstrap-select-searchbox">' + searchbox + clearLink + selectAllLink + '</div>';
        ...
        render: function() {
            ...
            if (this.options.clearLinkText) {
                this.$clearLink.attr('value', this.options.clearLinkText);
            }

            if (this.options.selectAllText) {
                this.$selectAllLink.attr('value', this.options.selectAllText);
            }
        ...
        setSize: function() {
            ...
            if (this.options.size == 'auto') {
                ...
                var clearLinkHeight = this.options.clearLink ? 30 : 0;
                var liveSearchHeight = this.options.liveSearch ? 48 : 0;
                ...
                menu.css({ 'max-height': menuHeight + headerHeight + clearLinkHeight  + liveSearchHeight + 'px' });
        ...
        clickListener: function() {
            ...
            this.$menu.on('click', 'dt', $.proxy(function (e) {
                if (!this.multiple) {
                    return;
                }

                var clickedIndex = this.$menu.find("dt").index(e.currentTarget),
                ...
                var $optGroup = $optGroups.eq(clickedIndex);

                var $options = $optGroup.find("option");

                var selected = !$optGroup.data("selected");
                $optGroup.data("selected", selected);
            ...
            this.$menu.on('click', 'li', $.proxy(function(e) {
        ...
        liveSearchListener: function() {
            ...
            this.$searchbox.on('input keyup', $.proxy(function () {
                this.$newElement.find('li').show().not('.selected').not(':icontains(' + this.$searchbox.val() + ')').hide();
            }, this));
        },

        clearLinkListener: function () {
            this.$clearLink.on('click', $.proxy(function (e) {
        ...
        selectAllLinkListener: function () {
            this.$selectAllLink.on('click', $.proxy(function (e) {
